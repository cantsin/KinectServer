...
namespace Microsoft.Samples.Kinect.Webserver
{
    ...
    public sealed class WebSocketRpcChannel : WebSocketChannelBase
    {
        ...
        public override bool CheckConnectionStatus()
        {
            ...
            {
                Trace.TraceError("base.CheckConnectionStatus (isOpen) returned false");
                return false;
            }

            if (this.isInCall)
            {
                Trace.TraceError("in call -- everything copaectic");
                return true;
            }
            ...
            Trace.TraceError("success was {0} and result was {1}", result.Success, result.Result);
